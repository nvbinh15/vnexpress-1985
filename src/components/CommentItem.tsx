 ...
 * "now" of 1985-09-14T18:00 — the satirical edition's publish moment —
 * so the relative strings are stable on every load.
 */
const NOW_1985 = new Date('1985-09-14T18:00:00+07:00').getTime()

function relTime(iso: string): string {
  const t = new Date(iso).getTime()
  if (Number.isNaN(t)) return ''
  const diff = Math.max(0, NOW_1985 - t)
